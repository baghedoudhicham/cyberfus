...
const RuntimeShield = () => {
  ...
  useEffect(() => {
    if (user) {
      const q = query(collection(db, 'runtime-events'), orderBy('timestamp', 'desc'));
  ...
  const simulateEvent = async () => {
    ...
    try {
      const eventTypes = ['SQL Injection', 'Cross-Site Scripting (XSS)', 'Remote Code Execution', 'File Inclusion'];
      const randomType = eventTypes[Math.floor(Math.random() * eventTypes.length)];
      const randomIp = `192.168.1.${Math.floor(Math.random() * 255)}`;

      await addDoc(collection(db, 'runtime-events'), {
        type: randomType,
        sourceIp: randomIp,
        status: 'Blocked',
        ...
        <main className="container mx-auto px-6 py-12">
          <div className="bg-gray-800 p-8 rounded-lg shadow-lg max-w-6xl mx-auto">
            <div className="flex justify-between items-center mb-8">
              <div>
                <h1 className="text-3xl font-bold">Runtime Shield</h1>
                <p className="text-gray-400">Real-time application security events</p>
              </div>
              <button
                ...
                <tbody>
                  {events.map(event => (
                    <tr key={event.id} className="border-b border-gray-800 hover:bg-gray-600">
                      ...
                      <td className="p-4 font-mono">{event.sourceIp}</td>
                      <td className="p-4"><span className="bg-red-500 text-white px-2 py-1 rounded-full text-xs">{event.status}</span></td>
